...
export function getTemplateFiles(directory: string): string[] {
    ...
    return files.filter(file => path.extname(file).toLowerCase() === '.template').map(file => path.join(directory, file));
}

export function getAvailablePresets(presetsDirectory: string | null = null): { [key: string]: string[] } {
    presetsDirectory = presetsDirectory || path.join(__dirname, '../../', 'presets');
    const files = fs.readdirSync(presetsDirectory);
    if (!files.length) {
        throw new Error(`No preset directories found in ${presetsDirectory}`);
    }
    return files
        .filter(file => fs.statSync(path.join(presetsDirectory, file)).isDirectory())
        .reduce((acc, file) => {
...
export async function askUser(question: string, defaultValue: string = ''): Promise<string> {
...
async function initWebpackConfig(
    ...
    const templateVars: { [key: string]: string } = {
        namespace: await askUser('What is the namespace you would like to use?', path.basename(currentWorkingDir))
    };
    ...
    for (const templateFile of availablePresets[preset]) {
        ...
        const destinationFile = path.join(process.cwd(), path.basename(templateFile.replace(/\.template$/, '')));
...
const argv = yargs(hideBin(process.argv)).command(
    ...
    () => console.log(`Available presets: ${Object.keys(getAvailablePresets()).join(', ')}`),
).option('help', {
    alias: 'h',
    describe: 'Show help',
    type: 'boolean',
